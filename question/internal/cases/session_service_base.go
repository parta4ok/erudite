...
func (srv *SessionServiceBase) ShowTopics(ctx context.Context) ([]string, error) {
	...
	if err != nil {
		slog.Error("GetTopics", "error", err.Error())
		span.SetError(err)
...
func (srv *SessionServiceBase) CreateSession(ctx context.Context, userID string,
	...
	if err != nil {
		slog.Error("NewSession", "error", err.Error())
		span.SetError(err)
	...
	if err != nil {
		slog.Error("IsDailySessionLimitReached", "error", err.Error())
		span.SetError(err)
	...
	if err != nil {
		slog.Error("GetQuestions", "error", err.Error())
		span.SetError(err)
	...
	if err = session.SetQuestions(
		...
		slog.Error("SetQuestions", "error", err.Error())
...
func (srv *SessionServiceBase) CompleteSession(
	...
	if err != nil {
		slog.Error("GetSessionBySessionID", "error", err.Error())
		span.SetError(err)
	...
	if err != nil {
		slog.Error("GetSessionResult", "error", err.Error())
		span.SetError(err)
...
func (srv *SessionServiceBase) GetAllCompletedUserSessions(ctx context.Context, userID string) (
	...
	if userID == "" {
		...
		slog.Error("userID not set", "error", err.Error())
		span.SetError(err)
		return nil, err
	}
	...
	if err != nil {
		...
		slog.Error("get all completed user sessions failure", "error", err.Error())
		span.SetError(err)
		return nil, err
	}
...
func (srv *SessionServiceBase) GetPassedStudentsTopics(ctx context.Context, students []string) (
	...
	if err != nil {
		...
		slog.Error("GetPassedUserTopics failure", "error", err.Error())
		span.SetError(err)
		return nil, err
	}
