...
func (srv *AuthService) GetMentorGroups(ctx context.Context, mentorID string) (
	...
	if err != nil {
		...
		span.SetError(err)
		slog.Error("GetMentorGroups failure", "error", err)
		return nil, err
	}
...
func (srv *AuthService) GetLinkedUsers(ctx context.Context, id string,
	...
	if err != nil {
		...
		span.SetError(err)
		slog.Error("GetLinkedUsers failure", "error", err)
		return nil, err
	}
	...
	if linkedUsers.Recipient == nil || linkedUsers.Student == nil {
		...
		span.SetError(err)
		slog.Error("nil users info", "error", err)
		return nil, err
	}
...
func (srv *AuthService) GetUserByID(ctx context.Context, userID string) (*entities.User, error) {
	...
	if err != nil {
		...
		span.SetError(err)
		slog.Error("GetUserByID failure", "error", err)
		return nil, err
	}
...
func (srv *AuthService) GetGroupTitleByID(ctx context.Context, groupID string) (string, error) {
	...
	if err != nil {
		...
		span.SetError(err)
		slog.Error("GetGroupTitleByID failure", "error", err)
		return "", err
	}
