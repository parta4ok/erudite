...
func (p *Provider) Generate(user *entities.User) (string, error) {
	...
	if err != nil {
		...
		slog.Error("signed of jwt failure", "error", err)
		return "", err
	}
...
func (p *Provider) Introspect(tokenString string) (*entities.UserClaims, error) {
	...
	if err != nil {
		...
		slog.Error("jwt parse failure", "error", err)
		return nil, err
	}

	if !token.Valid {
		err = errors.Wrapf(entities.ErrInvalidJWT, "jwt is invalid")
		slog.Error("jwt is invalid", "error", err)
		return nil, err
	}
	...
	if !ok {
		...
		slog.Error("extract claims failure", "error", err)
		return nil, err
	}
